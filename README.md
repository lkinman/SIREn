...
Additionally, in case the predictions are not accurate enough, the user may wish to try annotating the ground truth binarization thresholds for ~20-40 maps and fine-tuning the pre-trained model (weights.tar.gz provided above) on a selected number of maps. Note that the fine-tuning functionality has not been extensively tested. Normalized labels (normalized_labels.csv) are generated by ```siren preprocess``` upon providing template.csv to --labels.
...
siren fine_tune --voldir cnn_outputs/normalized/ --labels_csv cnn_outputs/normalized_labels.csv --num_epochs 5 --weights weights_5e6.pth --outdir fine_tuned_cnn
