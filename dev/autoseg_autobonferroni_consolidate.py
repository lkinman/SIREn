...
def main(args):
    ...
    sample_num = len(union_voxels)
    vals = range(0, sample_num)
    ...
    for j,i in enumerate(combinations(vals, 2)):
        vox1, vox2 = i 
    ...
    print('consolidating communities')
    blocks_dict_consolidated = consolidate_blocks(blocks_dict)
    utils.save_pkl(blocks_dict, outdir + 'blocks_dict.pkl')
    utils.save_pkl(blocks_dict, outdir + 'blocks_dict_consolidated.pkl')

    print('Bootstrapping p-values for community expansion')
    block_lens = [len(blocks_dict_consolidated[i]) for i in blocks_dict_consolidated.keys()]
    corr_factor_exp = len(vals)*np.sum(block_lens)
    posp_corr_exp = 1-(args.posp/corr_factor_exp)
    negp_corr_exp = 1-(args.negp/corr_factor_exp)
    ...
    utils.save_pkl(cutoffs_dict_exp, outdir + 'cutoffs_dict_exp.pkl')

    print('expanding communities with full voxel list')
    blocks_dict_expand = copy.deepcopy(blocks_dict_consolidated)
    unadded_list = []

    for i in vals:
        unadded = True
        x = binned[:, i].astype('float')
        for j in blocks_dict_consolidated.keys():
            counter = 0
            for k in blocks_dict_consolidated[j]:
                ...
                if (len(np.where(summed == 2)[0]) > 2*pos_cutoff) and (len(np.where(summed == 0)[0]) > neg_cutoff): 
                    counter += 1
            if counter/len(blocks_dict_consolidated[j]) >= args.exp_frac:
                blocks_dict_expand[j].append(i) 
                unadded = False
        if unadded:
            unadded_list.append(i)
    
    blocks_dict_expand_consolidated = consolidate_blocks(blocks_dict_expand)

    print('writing volumes and saving data')
    for i in blocks_dict_expand_consolidated.keys():
        write_vol(i, blocks_dict_expand_consolidated, outdir, vol_list, union_voxels)
    utils.save_pkl(blocks_dict_expand, outdir + 'blocks_dict_expand.pkl')
    utils.save_pkl(blocks_dict_expand_consolidated, outdir + 'blocks_dict_expand_consolidated.pkl')
